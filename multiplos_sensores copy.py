...
class SensorManager:
    ...
    def init_sensor(self, sensor_type, transform, attached, sensor_options):
        if sensor_type == 'RGBCamera':
            ...
            camera = self.world.spawn_actor(camera_bp, transform, attach_to=attached)
            camera.listen(self.save_rgb_image2)

            return camera

        elif sensor_type == 'SegmentationMask':
            camera_bp = self.world.get_blueprint_library().find('sensor.camera.semantic_segmentation')
            disp_size = self.display_man.get_display_size()
            camera_bp.set_attribute('image_size_x', str(disp_size[0]))
            camera_bp.set_attribute('image_size_y', str(disp_size[1]))
            ...
            camera = self.world.spawn_actor(camera_bp, transform, attach_to=attached)
            camera.listen(self.save_rgb_image3)
    ...
    def save_rgb_image2(self, image):
        t_start = self.timer.time()
    ...
    def save_rgb_image3(self, image): # imagem em p e b
        t_start = self.timer.time()
        ...
        array = np.frombuffer(image.raw_data, dtype=np.dtype("uint8"))
        array = np.reshape(array, (image.height, image.width, 4))
        #print(array.shape) #720 x 640 x 4 canais (BGRA)
        
        array = array[:, :, :3]
        array = process_img(array)
        #array = array[:, :, ::-1]
...
def run_simulation(args, client):
    ...
    try:
        ...
        #bp = world.get_blueprint_library().filter('charger2020')
        bp = random.choice(world.get_blueprint_library().filter('vehicle'))
        vehicle = world.spawn_actor(bp, 'Transform(Location(x=402.525452, y=-124.737938, z=0.281942), Rotation(pitch=0.000000, yaw=-89.401421, roll=0.000000))')
        ...
        SensorManager(world, display_manager, 'SegmentationMask', carla.Transform(carla.Location(x=0, z=2.4), carla.Rotation(yaw=00)), 
